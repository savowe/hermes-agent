...
2. **Git pull** — fast-forwards from the configured remote branch and updates submodules. If local and remote history diverged, the update stops without changing local commits and prints the manual merge commands; it never resets the checkout to the remote tip.
